...
unsigned char iap_program_data_byte(unsigned char val, unsigned int addr)
{
	EA=0;

	IAPFDATA = 0x55;		// Sequence to allow writing to IAPFCTRL
	IAPFDATA = 0xAA;
	IAPFDATA = 0x55;


	SYSCON = SYSCON | 0x4;  // Enable the IAP bit of SYSCON
	IAPFADHI = addr>>8;	// Set the address 
	IAPFADLO = addr;
	IAPFDATA = val;			// Set the data 
	ISPFCTRL = 0x80;		// Begin writing
...
void iap_erase_block(unsigned char block)
{
	...
	SYSCON = SYSCON | 0x4;  // Enable the IAP bit of SYSCON
	IAPFADHI = block<<1;		// Set the address 
	IAPFADLO = 0x00;
	ISPFCTRL = 0x82;		// Begin erasing

	EA=1;
}


char FlashRead_Absolute(unsigned int location, unsigned char *value)
{
...
bit flash_set_block(unsigned char block_select)
{
	...
	block_base1 = (unsigned char code *)((unsigned int)block1 << 9);
	block_base2 = (unsigned char code *)((unsigned int)block2 << 9);
...
unsigned char flash_garbage_collector(unsigned char block_select)
{
	...
	code_ptr = (unsigned char code*)( (unsigned int)flash_unused_block[block_select] << 9 );

	block_size = BLOCK_SIZE_CONST[block_select];

	// erase unused block
	for(b=0; b<(BLOCK_SIZE_CONST[block_select]>>9); b++)
	{
		watchdog();
...
// debug, for some reason the first one always gets deleted
	while((*scan_ptr != 0xff) && (scan_ptr < temp_ptr))
	{
		...
		if (*(scan_ptr + 1) != 0x00 && *(scan_ptr + 1) != 0xff)
		{
			...
			// copy n
			n = *scan_ptr;
			if (iap_program_data_byte(n, (unsigned int)code_ptr))
	...
	unused_block = (unsigned char)(block_base[block_select] >> 9);
	block_base[block_select] = (unsigned int)(flash_unused_block[block_select] << 9);
...
//*************************************************************************
	...
	if (iap_program_data_byte(FLASH_BLOCK_INVALID, (unsigned int)(flash_unused_block[block_select] << 9)))
	return FLASH_PROG_ERROR;
...
//  old_entry_id = (unsigned int code *)0;
	...
	flash_ptr[block_select] += 1;  // increase flash_ptr by size + 1
